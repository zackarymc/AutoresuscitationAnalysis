...
% rootDir= 'C:\Users\zcobu\OneDrive\Desktop\VA\SIDS\Code';
% respDataFileName='DATA_1.mat';
% 
% % sample data contains 
% respiratoryData = load(fullfile(rootDir,respDataFileName));

respiratoryRecording = resp(2,1:90000);
srate = 1000;
dataType = 'rodentAirflow';

bmObj = customBreathmetrics(respiratoryRecording, srate, dataType,0);
bmObj.estimateAllFeatures(0,'done',1,1);

IBI = diff(bmObj.inhaleOnsets)./1000;
irregularity = zeros(1,length(IBI)-1);

for i = 1:length(irregularity)
    irregularity(i) = (abs(IBI(i+1)-IBI(i))/IBI(i))*100;
end
