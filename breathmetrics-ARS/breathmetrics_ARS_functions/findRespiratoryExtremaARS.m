function [correctedPeaks, correctedTroughs] = findRespiratoryExtremaARS( ...
...
while pki <length(peakInds)-1 && tri<length(troughInds)-1
    ...
    if peakPeakDiff < peakTroughDiff
        % This is a pure inspiratory effort (breath packing)
        % Mark the index of this breath to mark the lack of an exhale

        peakToPeak = paddedResp(peakInds(pki):peakInds(pki+1));
        if isempty(find(peakToPeak <= SIMPLE_ZERO_CROSS, 1))

            % Forget this peak, move to the next one
            ...
            peakDiff = diff([paddedResp(peakInds(pki)),...
                paddedResp(peakInds(pki+1))]);
            peakDist = peakInds(pki+1)-peakInds(pki);

            peakHeight = paddedResp(peakInds(pki));
            meanPeakHeight = mean(resp(correctedPeaks));

            if (peakDiff > meanPeakHeight/2) && peakHeight<(meanPeakHeight/2) && peakDist<100
                pki = pki+1;
                proceedCheck = 0;
            else
                packingBreaths(packInd) = peakInds(pki);
                packInd = packInd+1;
    ...
    if proceedCheck == 1
        ...
        if peakTroughDiff > 0
            ...
            if peakPeakDiff < peakTroughDiff
                ...
                correctedTroughs = [correctedTroughs 1];
                pki = pki+1;
