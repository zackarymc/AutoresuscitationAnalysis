function respirationStatistics  = getSecondaryRespiratoryFeaturesARS( Bm, verbose )
...
if strcmp(Bm.dataType,'humanAirflow') || strcmp(Bm.dataType,'rodentAirflow')
    ...
    avgInhaleDuration = mean(Bm.inhaleDurations,'omitnan');
    avgExhaleDuration = mean(Bm.exhaleDurations,'omitnan');
    ...
    avgInhalePauseDuration = mean(Bm.inhalePauseDurations(validInhaleInds),'omitnan') * pctInhalePause;
    
    pctExhalePause=sum(~isnan(Bm.exhalePauseDurations))/nValidExhales;
    avgExhalePauseDuration = mean(Bm.exhalePauseDurations(validExhaleInds)) * pctExhalePause;
    ...
    CVInhaleDuration = std(Bm.inhaleDurations,'omitnan')/avgInhaleDuration;
    CVInhalePauseDuration = std(Bm.inhalePauseDurations,'omitnan')/avgInhalePauseDuration;
    CVExhaleDuration = std(Bm.exhaleDurations,'omitnan')/avgExhaleDuration;
    CVExhalePauseDuration = std(Bm.exhalePauseDurations,'omitnan')/avgExhalePauseDuration;
...
function validVals=excludeOutliers(origVals,validBreathInds)
    ...
    upperBound=mean(origVals,'omitnan') + 2 * std(origVals,'omitnan');
    lowerBound=mean(origVals,'omitnan') - 2 * std(origVals,'omitnan');
