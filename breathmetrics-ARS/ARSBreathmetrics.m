 classdef ARSBreathmetrics < matlab.mixin.Copyable
    ...
    % smoothType: Determines if the data will be smoothed.
    % sectionType: Defines if the section is from baseline or a challenge.
    ...
    methods
        %%% constructor %%%
        function Bm = ARSBreathmetrics(resp, srate, dataType,smoothType)
            ...
            Bm.smoothType = smoothType;

            Bm.checkClassInputs();
            
            if smoothType == 1 % if resp needs to be smoothed
            ...
            elseif smoothType == 0 % if resp is already smoothed
        ...
        function Bm = correctRespirationToBaseline(Bm, method, ...
            ...
            %    'done' : if resp has already been baseline corrected. 
            ...
            elseif strcmp(method,'done')
                % for data already baseline corrected
                Bm.baselineCorrectedRespiration = Bm.smoothedRespiration;
        ...
        function Bm = findExtremaARS(Bm, simplify, verbose)
            ...
            if strfind(Bm.dataType, 'human')==1
                [putativePeaks, putativeTroughs] = ...
                    findRespiratoryExtremaARS( thisResp, Bm.srate );
                
            % same extrema finding for thermocouple and airflow recordings
            % in rodents
            elseif strfind(Bm.dataType, 'rodent')==1
                srateAdjust = Bm.srate/1000;
                rodentSWSizes = [floor(20*srateAdjust), ...
                    floor(50 * srateAdjust), floor(100 * srateAdjust), ...
                    floor(250 * srateAdjust), floor(500 * srateAdjust)];
                [putativePeaks, putativeTroughs] = ...
                    findRespiratoryExtremaARS( thisResp, Bm.srate, ...
                    0, rodentSWSizes );
            end
        ...
        function Bm = findOnsetsAndPauses(Bm, verbose )
            ...
            [theseInhaleOnsets, theseExhaleOnsets, ...
                ...
                findRespiratoryPausesAndOnsetsARS(thisResp, ...
        ...
        function Bm = findInhaleAndExhaleOffsets(Bm, verbose )
            ...
            [inhaleOffs,exhaleOffs]=findRespiratoryOffsetsARS(thisResp, ...
        ...
        function Bm = findBreathAndPauseDurations(Bm)
            ...
            [inhaleDurs, exhaleDurs, inhalePauseDurs, ...
                exhalePauseDurs] = findBreathDurationsARS(Bm);
        ...
        function Bm = findInhaleAndExhaleVolumes(Bm, verbose )
            ...
            [inhaleVols,exhaleVols] = findRespiratoryVolumesARS( ...
        ...
        function Bm = getSecondaryFeatures( Bm, verbose )
            ...
            respStats  = getSecondaryRespiratoryFeaturesARS( Bm, verbose );
        ...
        function Bm = estimateAllFeatures( Bm, zScore, ...
            ...
            if strcmp(Bm.dataType,'humanAirflow') || ...
                ...
                Bm.correctRespirationToBaseline(baselineCorrectionMethod, ...
                    zScore, verbose);
                Bm.findExtremaARS(simplify, verbose);
